...
def conv2d_grayscale(img, ker):
    ...
    for x in range(Nx):
        for y in range(Ny):
            feature_map[y, x] = np.sum(np.multiply(img[y:y+j, x:x+i], ker))
    # !!! ==============
...
def conv2d_many(img, kers):
    ...
    #
    # !!! YOUR CODE HERE
    feature_maps = [conv2d(img, k) for k in kers]
    # !!! ==============
...
def conv2d_full(img, kers, stride=1):
    ...
    for ker in kers:
        ...
        for x in range(Nx):
            for y in range(Ny):
                feature_map[y, x] = np.sum(np.multiply(img[y*stride : y*stride+j, x*stride : x*stride+i], ker))
        # !!! ==============
